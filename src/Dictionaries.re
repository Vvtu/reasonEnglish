...
let dictionary1 = [
  ...
  {rus: {js|События кроме спринта могут заканчиваться всякий раз, когда цель события достигается.|js},
   eng: "Other than the Sprint events may end whenever the purpose of the event is achived."},
  ...
  {rus: {js|Каждый спринт имеет цель того, что должно быть построено, дизайн и гибкий план, который будет направлять его создание, работу и результирующее увеличение продукта.|js},
   eng: "Each Sprint has a goal of what is to be build, a design and flexible plan that will guide building it, the work, and the resultant product increment."},
